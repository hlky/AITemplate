...
class MeshGridTestCase(unittest.TestCase):
    def _test_single_op(
        ...
        for out_ait, out_pt in zip(output_data, output_tensors):
            torch.testing.assert_close(
                out_ait,
                out_pt.to(out_ait.dtype),
                rtol=1e-3,
                atol=1e-3,
    ...
    def test_meshgrid(self, ait_dtype):
        self._test_single_op(
            input_shapes=[[4], [5], [6]],
            indexing="ij",
            test_name=f"meshgrid_{ait_dtype}_ij",
            dtype=ait_dtype,
        )
        self._test_single_op(
            input_shapes=[[10], [20]],
        ...
        self._test_single_op(
            input_shapes=[[10], [20]],
            indexing="xy",
            test_name=f"meshgrid_{ait_dtype}_xy",
            dtype=ait_dtype,
        )
